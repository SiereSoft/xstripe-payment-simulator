...
class ControlPlaneIsolationTest {
    ...
    @Test
    fun `default listeners expose actor publicly and controller only on loopback`() {
        val config = ServerConfig.from(emptyMap())

        assertEquals("0.0.0.0", config.actorHost)
    ...
    @Test
    fun `emulator-facing actor can use provider API but cannot route control calls`() = testApplication {
        ...
        assertEquals(
            ...
            client.post("/v1/customers") {
                header(HttpHeaders.Authorization, "Bearer sk_test_android")
                setBody(FormDataContent(parametersOf("email" to listOf("android@example.com"))))
            }.status,
        )
